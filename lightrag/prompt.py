...
You are a Knowledge Graph Specialist responsible for extracting entities from the input text, which is from the game Wuthering Waves.
...
5.  **Completion Signal:** Output the literal string `{completion_delimiter}` only after all entities, following all criteria, have been completely extracted and outputted.
...
Based on the last extraction task, identify and extract any **missed or incorrectly formatted** entities from the input text.

---Instructions---
1.  **Strict Adherence to System Format:** Strictly adhere to all format requirements for entity lists, including output order, field delimiters, and proper noun handling, as specified in the system instructions.
2.  **Focus on Corrections/Additions:**
    *   **Do NOT** re-output entities that were **correctly and fully** extracted in the last task.
    *   If an entity was **missed** in the last task, extract and output it now according to the system format.
    *   If an entity was **truncated, had missing fields, or was otherwise incorrectly formatted** in the last task, re-output the *corrected and complete* version in the specified format.
3.  **Output Format - Entities:** Output a total of 4 fields for each entity, delimited by `{tuple_delimiter}`, on a single line. The first field *must* be the literal string `entity`.
4.  **Output Content Only:** Output *only* the extracted list of entities. Do not include any introductory or concluding remarks, explanations, or additional text before or after the list.
5.  **Completion Signal:** Output `{completion_delimiter}` as the final line after all relevant missing or corrected entities have been extracted and presented.
